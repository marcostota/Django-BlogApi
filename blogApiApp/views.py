...
def create_post(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response({"Succers': 'The post was created"}, status=201)
    else:
...
def delete_post(request):
    ...
    try:
        ...
        post.delete()
        return Response({'Success':'Post successfully deleted'}, status=401)
    except Post.DoesNotExist:
        return Response({'Erro':'The post does not exists'}, status=404)
...
def update_post(request):
    ...
    get_new_content = request.data.get('l')
